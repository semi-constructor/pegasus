...
export async function handleTradeButtons(interaction: ButtonInteraction) {
  ...
  if (action === 'accept') {
    ...
    const initiatorBalanceResult = await db
      .select()
      ...
      .limit(1);
    const receiverBalanceResult = await db
      .select()
      .from(economyBalances)
      .where(
        and(
          eq(economyBalances.userId, trade.receiverId),
          eq(economyBalances.guildId, trade.guildId)
        )
      )
      .limit(1);
    ...
    if (
      initiatorOfferCoins > 0 &&
      initiatorBalance &&
      initiatorBalance.balance >= initiatorOfferCoins
    ) {
      await db
    ...
    if (
      receiverOfferCoins > 0 &&
      receiverBalance &&
      receiverBalance.balance >= receiverOfferCoins
    ) {
      await db
        .update(economyBalances)
    ...
    await interaction.update({
      content: 'Trade accepted and completed!',
      embeds: [],
      components: [],
    });
  }
      ...
      .addFields(
        {
          ...
          value: `Coins: ${(newTrade.initiatorOffer as any)?.coins || 0}`,
          inline: true,
        },
        {
          name: `Receiver's Offer`,
          value: `Coins: ${(newTrade.receiverOffer as any)?.coins || 0}`,
          inline: true,
        }
      )
      .setColor(0x3498db)
