...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand
      .setName('available')
      .setDescription(t('commands.language.subcommands.available.description', { defaultValue: 'List available languages' }))
      .setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.language.available))
  )
  .addSubcommand(subcommand =>
    subcommand
      .setName('current')
      .setDescription(t('commands.language.subcommands.current.description', { defaultValue: 'Show current language' }))
      .setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.language.current))
  )
  .addSubcommand(subcommand =>
    subcommand
      .setName('set')
      .setDescription(t('commands.language.subcommands.set.description', { defaultValue: 'Set preferred language' }))
      .setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.language.set))
      .addStringOption(option =>
        option
          .setName('language')
          .setDescription(t('commands.language.subcommands.set.options.language', { defaultValue: 'The language to select' }))
          ...
          .addChoices(
            ...
            { name: 'Deutsch', value: 'de', name_localizations: { de: 'Deutsch', 'es-ES': 'Alemán', fr: 'Allemand' } },
            { name: 'Español', value: 'es', name_localizations: { de: 'Spanisch', 'es-ES': 'Español', fr: 'Espagnol' } },
            { name: 'Français', value: 'fr', name_localizations: { de: 'Französisch', 'es-ES': 'Francés', fr: 'Français' } }
          )
      )
  );
...
async function handleAvailable(interaction: ChatInputCommandInteraction, locale: string) {
  const embed = new EmbedBuilder()
    .setColor(0x0099ff)
    .setTitle(t('commands.language.subcommands.available.title', { lng: locale, defaultValue: 'Available Languages' }))
    .setDescription(t('commands.language.subcommands.available.description', { lng: locale, defaultValue: 'Here are the available languages:' }))
    .addFields(
      {
        name: t('commands.language.subcommands.available.fields.en.name', { lng: locale, defaultValue: '🇬🇧 English' }),
        value: t('commands.language.subcommands.available.fields.en.value', { lng: locale, defaultValue: 'Default language' }),
        inline: true,
      },
      {
        name: t('commands.language.subcommands.available.fields.de.name', { lng: locale, defaultValue: '🇩🇪 Deutsch' }),
        value: t('commands.language.subcommands.available.fields.de.value', { lng: locale, defaultValue: 'German language' }),
        inline: true,
      },
      {
        name: t('commands.language.subcommands.available.fields.es.name', { lng: locale, defaultValue: '🇪🇸 Español' }),
        value: t('commands.language.subcommands.available.fields.es.value', { lng: locale, defaultValue: 'Spanish language' }),
        inline: true,
      },
      {
        name: t('commands.language.subcommands.available.fields.fr.name', { lng: locale, defaultValue: '🇫🇷 Français' }),
        value: t('commands.language.subcommands.available.fields.fr.value', { lng: locale, defaultValue: 'French language' }),
        inline: true,
      }
    )
    .setFooter({
      text: t('commands.language.subcommands.available.footer', { lng: locale, defaultValue: 'Use /language set <language> to change your language' }),
    })
    .setTimestamp();
...
async function handleCurrent(interaction: ChatInputCommandInteraction, currentLocale: string) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0x0099ff)
    .setTitle(t('commands.language.subcommands.current.title', { lng: currentLocale, defaultValue: 'Current Language' }))
    .setDescription(
    ...
    .setFooter({
      text: t('commands.language.subcommands.current.footer', { lng: currentLocale, defaultValue: 'Use /language set <language> to change your language' }),
    })
    .setTimestamp();
...
async function handleSet(interaction: ChatInputCommandInteraction, initialLocale: string) {
  ...
  if (!availableLocales.includes(newLocale)) {
    return interaction.editReply({
      content: t('commands.language.subcommands.set.invalidLanguage', { lng: initialLocale, defaultValue: 'Invalid language selected.' }),
    });
  }
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00ff00)
      .setTitle(t('commands.language.subcommands.set.success.title', { lng: newLocale, defaultValue: 'Language Updated' }))
      .setDescription(
